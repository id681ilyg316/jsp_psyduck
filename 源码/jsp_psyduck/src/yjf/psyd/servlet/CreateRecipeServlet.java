				...
				if (item.isFormField()) {
					...
					if (fileName != null && !"".equals(fileName)) {
						if (itemName.equals("recCover")) {
							// 获取文件内容 并上传
							String path = Thread.currentThread().getContextClassLoader().getResource("").getPath().substring(0,Thread.currentThread().getContextClassLoader().getResource("").getPath().length()-16)+"\\upload\\cover";
							// 使用uuid生成随机数，调用工具类
...
//								System.out.println("封面上传成功！路径：" + coverFilePath);
							...
							String path = Thread.currentThread().getContextClassLoader().getResource("").getPath().substring(0,Thread.currentThread().getContextClassLoader().getResource("").getPath().length()-16)+"\\upload\\stepFile";
							// 使用uuid生成随机数，调用工具类
							String uuidFileName = UploadTools.getUUIDFileName(fileName);
